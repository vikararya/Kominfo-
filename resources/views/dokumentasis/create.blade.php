...
<head>
    ...
    <style>
        .readonly-editor {
    background-color: black !important; /* Warna abu-abu */
    pointer-events: none; /* Menghilangkan interaksi (klik dan cursor) */
    cursor: default !important; /* Menghilangkan kursor */
}
                                ...
                                <div class="card-body">
                                    <form action="{{ route('dokumentasis.store') }}" method="POST" enctype="multipart/form-data">
                                        ...
                                        <div class="form-group">
                                            <label>Cover:</label>
                                                <select name="cover_id" id="cover" class="form-control" required>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div>
                                                <label class="mr-3">
                                                    <input type="radio" name="deskripsi_status" value="aktif" checked> Aktif (Tidak bisa mengisi deskripsi)
                                                </label>
                                                <label>
                                                    <input type="radio" name="deskripsi_status" value="non-aktif"> Non-Aktif (Bisa mengisi deskripsi)
                                                </label>
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <textarea class="form-control @error('deskripsi') is-invalid @enderror"
                                                id="deskripsi" name="deskripsi" rows="5" placeholder="Masukkan Deskripsi">{{ old('deskripsi') }}</textarea>
                                            @error('deskripsi')
                                        ...
                                        <!-- Dropdown Versi -->
                                        <div class="form-group">
                                            <label for="versi_id">Pilih Versi</label>
                                            <select name="versi_id" id="versi_id" class="form-control">
...
$(document).ready(function() {
    ...
    $('#cover').change(function() {
        ...
        var versiDropdown = $('#versi_id'); // Perbaiki ID agar sesuai dengan dropdown versi

        if (coverId) {
            $.ajax({
                url: '/get-versions/' + coverId,
                type: 'GET',
                success: function(response) {
                    versiDropdown.prop('disabled', false);
                    versiDropdown.empty().append('<option value="">-- Pilih Versi --</option>');

                    if (response.length > 0) {
                        $.each(response, function(index, versi) {
                            versiDropdown.append('<option value="' + versi.id + '">' + versi.versi + '</option>');
                        });
                    } else {
                        versiDropdown.append('<option value="">Belum ada versi</option>');
                    }
                },
                error: function() {
                    alert('Gagal mengambil versi');
                }
            });
        } else {
            versiDropdown.prop('disabled', true);
            versiDropdown.empty().append('<option value="">-- Pilih Versi --</option>');
        }
    });
});
